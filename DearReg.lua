...
local Templates = {
    ...
    Window = {
        ...
        IconSize = UDim2.fromOffset(64, 64),
        BackgroundTransparency = 0,
        AutoShow = true,
        Center = true,
        Resizable = true,
...
function Library:GetIconColor(IconName: string)
    if Library.IconColors and Library.IconColors[IconName] then
        local iconColor = Library.IconColors[IconName]
        if typeof(iconColor) == "table" then
            return iconColor
        end
        return iconColor
    end
...
function Library:GetCustomIcon(IconName: string, CustomColor: Color3?)
    local IconColorData = Library:GetIconColor(IconName)
    local IconColor = CustomColor
    local CircleColor = nil
    local CircleTransparency = nil
    local Shape = nil
    
    if IconColorData then
        if typeof(IconColorData) == "Color3" then
            IconColor = IconColorData
        elseif typeof(IconColorData) == "table" then
            IconColor = IconColorData.IconColor or IconColorData.Color
            CircleColor = IconColorData.CircleColor
            CircleTransparency = IconColorData.CircleTransparency
            Shape = IconColorData.Shape
        end
    end
    ...
    if not IsValidCustomIcon(IconName) then
        ...
        if icon then
            ...
            if CircleColor then
                icon.CircleColor = CircleColor
            end
            if CircleTransparency then
                icon.CircleTransparency = CircleTransparency
            end
            if Shape then
                icon.Shape = Shape
            end
        ...
        local ColorMenu = Library:AddContextMenu(
            ...
            function()
                if Library.IsMobile then
                    ...
                    local MenuWidth = Info.Transparency and 256 or 234
                    local MenuHeight = 280
                    return { 
                        (ViewportSize.X - MenuWidth * Library.DPIScale) / 2 - Holder.AbsolutePosition.X,
                        (ViewportSize.Y - MenuHeight * Library.DPIScale) / 2 - Holder.AbsolutePosition.Y
                    }
                else
...
do
    ...
    function Funcs:AddLabel(...)
        ...
        if typeof(First) == "table" or typeof(Second) == "table" then
            ...
            Data.AutoReset = Params.AutoReset
        else
            Data.Text = First or ""
        ...
        if Label.Icon then
            local IconImage = Library:GetCustomIcon(Label.Icon)
            if IconImage then
                New("ImageLabel", {
                    Image = IconImage.Url,
                    ImageColor3 = IconImage.Color or (IconImage.Custom and "White" or "FontColor"),
                    ImageRectOffset = IconImage.ImageRectOffset,
                    ImageRectSize = IconImage.ImageRectSize,
                    Size = UDim2.fromOffset(14, 14),
                    Parent = Holder,
                })
            elseif typeof(Label.Icon) == "number" then
                New("ImageLabel", {
                    Image = "rbxassetid://" .. Label.Icon,
                    Size = UDim2.fromOffset(14, 14),
                    Parent = Holder,
                })
            end
        ...
        local TextLabel = New("TextLabel", {
            BackgroundTransparency = 1,
            Size = UDim2.new(1, Label.Icon and -20 or 0, 1, 0),
            Text = Label.Text,
            TextSize = Data.Size,
        ...
        function Label:AddButton(ButtonInfo)
            local ButtonElement = Groupbox:AddButton(ButtonInfo)
            local OriginalFunc = ButtonElement.Func
            local OriginalText = Label.Text
            
            ButtonElement.Func = function()
                Label:SetText(ButtonInfo.Text .. " - Clicked!")
                OriginalFunc()
                task.wait(1)
        ...
        function Label:AddToggle(ToggleIdx, ToggleInfo)
            local ToggleElement = Groupbox:AddToggle(ToggleIdx, ToggleInfo)
            local OriginalCallback = ToggleElement.Callback
            local OriginalText = Label.Text
            
            ToggleElement.Callback = function(Value)
                Label:SetText(OriginalText .. " - " .. (Value and "ON" or "OFF"))
                OriginalCallback(Value)
            end
            
            ToggleElement:OnChanged(function(Value)
                Label:SetText(OriginalText .. " - " .. (Value and "ON" or "OFF"))
            end)
    ...
    function Funcs:AddButton(...)
        ...
        local function CreateButton(Button)
            ...
            if Button.Icon then
                local IconImage = Library:GetCustomIcon(Button.Icon)
                if IconImage then
                    New("ImageLabel", {
                        Image = IconImage.Url,
                        ImageColor3 = IconImage.Custom and "White" or "FontColor",
                        ImageRectOffset = IconImage.ImageRectOffset,
                        ImageRectSize = IconImage.ImageRectSize,
                ...
                elseif typeof(Button.Icon) == "number" then
                    New("ImageLabel", {
        ...
        local function InitEvents(Button)
            Button.Base.MouseEnter:Connect(function()
                ...
                for _, child in ipairs(Button.Base:GetDescendants()) do
                    if child:IsA("TextLabel") then
                        TweenService:Create(child, Library.TweenInfo, {
                            TextTransparency = 0,
                        }):Play()
                ...
                if Button.DoubleClick then
                    ...
                    if Clicked then
                        ...
                        for _, Element in pairs(Groupbox.Elements) do
                            if Element.Type == "Label" and Element.AutoReset then
                                for OptionIdx, Option in pairs(Library.Options) do
                                    if Option.Type == "Dropdown" and table.find(Element.AutoReset, OptionIdx) then
                                        Option:Reset()
                                    end
                                end
                            end
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        if Toggle.Icon then
            local IconImage = Library:GetCustomIcon(Toggle.Icon)
            if IconImage then
                New("ImageLabel", {
                    Image = IconImage.Url,
                    ImageColor3 = IconImage.Custom and "White" or "FontColor",
            ...
            elseif typeof(Toggle.Icon) == "number" then
                New("ImageLabel", {
                    Image = "rbxassetid://" .. Toggle.Icon,
                    ImageTransparency = 0.4,
                    Size = UDim2.fromOffset(16, 16),
                    Parent = Button,
                })
        ...
        local Label = New("TextLabel", {
            BackgroundTransparency = 1,
            Size = UDim2.new(1, -40 - (Toggle.Icon and 22 or 0), 1, 0),
            Text = Toggle.Text,
            TextSize = 14,
            TextTransparency = 0.4,
            TextXAlignment = Enum.TextXAlignment.Left,
    ...
    function Funcs:AddInput(Idx, Info)
        ...
        if Input.Icon then
            local IconImage = Library:GetCustomIcon(Input.Icon)
            if IconImage then
                New("ImageLabel", {
                    Image = IconImage.Url,
                    ImageColor3 = IconImage.Custom and "White" or "FontColor",
            ...
            elseif typeof(Input.Icon) == "number" then
                New("ImageLabel", {
                    Image = "rbxassetid://" .. Input.Icon,
                    Size = UDim2.fromOffset(14, 14),
                    Parent = Label,
                })
            end
        ...
        local LabelText = New("TextLabel", {
            BackgroundTransparency = 1,
            Size = UDim2.new(1, Input.Icon and -20 or 0, 1, 0),
            Text = Input.Text,
            TextSize = 14,
            TextXAlignment = Enum.TextXAlignment.Left,
            Parent = Label,
        ...
        function Input:UpdateColors()
            ...
            LabelText.TextTransparency = Input.Disabled and 0.8 or 0
    ...
    function Funcs:AddOutput(Info)
        ...
        local Output = {
            Lines = {},
            MaxLines = Info.MaxLines or 100,
            Text = Info.Text or "Output",
        ...
        local Holder = New("Frame", {
            BackgroundTransparency = 1,
            Size = UDim2.new(1, 0, 0, 18 + Output.Height),
            Parent = Container,
        })
        ...
        local OutputBox = New("Frame", {
            BackgroundColor3 = "MainColor",
            Position = UDim2.fromOffset(0, 20),
            Size = UDim2.new(1, 0, 0, Output.Height),
            Parent = Holder,
        })
        New("UICorner", {
            CornerRadius = UDim.new(0, Library.CornerRadius),
        ...
        function Output:AddLine(Text, Color, Icon)
            ...
            if Icon then
                ...
                if IconData then
                    New("ImageLabel", {
                        Image = IconData.Url,
                        ImageColor3 = Color or "FontColor",
            ...
            local Line = New("TextLabel", {
                BackgroundTransparency = 1,
                Size = UDim2.new(1, Icon and -16 or 0, 1, 0),
            ...
            table.insert(Output.Lines, LineFrame)
            
            -- Remove old lines if exceeding max
            if #Output.Lines > Output.MaxLines then
                local OldLine = table.remove(Output.Lines, 1)
                OldLine:Destroy()
            end
        ...
        function Output:Clear()
            for _, Line in pairs(Output.Lines) do
                Line:Destroy()
            end
            Output.Lines = {}
        end
        
        function Output:SetVisible(Visible)
            Holder.Visible = Visible
            Groupbox:Resize()
        end
        
        function Output:AttachToInput(Input)
            Input:OnChanged(function(Value)
                Output:AddLine("Input: " .. tostring(Value), Output.TextColor or Color3.fromRGB(100, 200, 255))
            end)
            return Output
        end
        
        function Output:AttachToToggle(Toggle)
            Toggle:OnChanged(function(Value)
                Output:AddLine("Toggle " .. (Value and "enabled" or "disabled"), Output.TextColor or Color3.fromRGB(50, 255, 100))
            end)
            return Output
        end
        
        function Output:AttachToButton(ButtonText, Callback)
            return function()
                Output:AddLine("Button clicked: " .. ButtonText, Output.TextColor or Color3.fromRGB(255, 180, 100))
                if Callback then
                    Callback()
                end
            end
        ...
        function Output:AttachToCheckbox(Checkbox)
            Checkbox:OnChanged(function(Value)
                Output:AddLine("Checkbox " .. (Value and "checked" or "unchecked"), Output.TextColor or Color3.fromRGB(150, 100, 255))
            end)
            return Output
        end
        
        function Output:AttachToColorPicker(ColorPicker)
            ColorPicker:OnChanged(function(Value)
                Output:AddLine("Color changed", Value)
                Output.TextColor = Value
            end)
            return Output
        end
        ...
        table.insert(Groupbox.Elements, Output)
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        local SliderLabel
        local SliderLabelHolder
        if not Info.Compact then
            SliderLabelHolder = New("Frame", {
                BackgroundTransparency = 1,
                Size = UDim2.new(1, 0, 0, 14),
                Parent = Holder,
            })
            ...
            if Slider.Icon then
                local IconImage = Library:GetCustomIcon(Slider.Icon)
                if IconImage then
                    New("ImageLabel", {
                        Image = IconImage.Url,
                        ImageColor3 = IconImage.Custom and "White" or "FontColor",
                ...
                elseif typeof(Slider.Icon) == "number" then
                    New("ImageLabel", {
                        Image = "rbxassetid://" .. Slider.Icon,
                        Size = UDim2.fromOffset(14, 14),
                        Parent = SliderLabelHolder,
                    })
                end
            ...
            SliderLabel = New("TextLabel", {
                BackgroundTransparency = 1,
                Size = UDim2.new(1, Slider.Icon and -20 or 0, 1, 0),
                Text = Slider.Text,
                TextSize = 14,
                TextXAlignment = Enum.TextXAlignment.Left,
                Parent = SliderLabelHolder,
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        function Dropdown:UpdateColors()
            ...
            ArrowImage.ImageTransparency = Dropdown.Disabled and 0.8 or 0
        end
        ...
        function Dropdown:Reset()
            if Info.Multi then
                Dropdown.Value = {}
                for i = 1, #Dropdown.Default do
                    local Index = Dropdown.Default[i]
                    Dropdown.Value[Dropdown.Values[Index]] = true
                end
            else
                if #Dropdown.Default > 0 then
    ...
    function Funcs:AddTabbox()
        ...
        function Tabbox:AddTab(Name, IconName)
            local HasText = Name ~= "" and Name ~= nil
            
            local Button = New("TextButton", {
                BackgroundColor3 = "MainColor",
                BackgroundTransparency = 0,
                Size = UDim2.fromOffset(0, 34),
                Text = (not IconName and HasText) and Name or "",
                TextSize = 15,
                TextTransparency = 0.5,
                Parent = TabboxButtons,
            })
            ...
            if IconName then
                ...
                if BoxIcon then
                    TabIcon = New("ImageLabel", {
                        Image = BoxIcon.Url,
                        ImageColor3 = BoxIcon.Color or (BoxIcon.Custom and "White" or "AccentColor"),
                        ImageRectOffset = BoxIcon.ImageRectOffset,
                        ImageRectSize = BoxIcon.ImageRectSize,
                        ImageTransparency = 0.5,
                        AnchorPoint = HasText and Vector2.new(0, 0.5) or Vector2.new(0.5, 0.5),
                        Position = HasText and UDim2.fromOffset(6, 17) or UDim2.fromScale(0.5, 0.5),
                ...
                if HasText then
                    local TextLabel = New("TextLabel", {
                        BackgroundTransparency = 1,
                        Position = UDim2.fromOffset(TabIcon and 32 or 6, 0),
                        Size = UDim2.new(1, TabIcon and -38 or -12, 1, 0),
                        Text = Name,
                        TextSize = 15,
                        TextTransparency = 0.5,
                        TextXAlignment = Enum.TextXAlignment.Left,
            ...
            Tabbox.Tabs[Name or tostring(#Tabbox.Tabs + 1)] = Tab
...
function Library:SetBackgroundTransparency(Transparency: number)
...
function Library:Notify(...)
    ...
    local IconOffset = HasIcon and 32 or 0
    
    New("UIPadding", {
        PaddingBottom = UDim.new(0, 8),
        PaddingLeft = UDim.new(0, 8 + IconOffset),
        PaddingRight = UDim.new(0, 8),
    ...
    if HasIcon then
        local NotifIcon = Library:GetCustomIcon(Data.Icon)
        if NotifIcon then
            New("ImageLabel", {
                Image = NotifIcon.Url,
                ImageColor3 = NotifIcon.Color or (NotifIcon.Custom and "White" or "AccentColor"),
                ImageRectOffset = NotifIcon.ImageRectOffset,
                ImageRectSize = NotifIcon.ImageRectSize,
                Position = UDim2.fromOffset(8, 8),
                Size = UDim2.fromOffset(24, 24),
                Parent = Holder,
            })
        elseif typeof(Data.Icon) == "number" then
            New("ImageLabel", {
    ...
    function Data:Resize()
        if Title then
            local X, Y = Library:GetTextBounds(
                ...
                Title.FontFace,
                Title.TextSize,
                NotificationArea.AbsoluteSize.X - ((24 + IconOffset) * Library.DPIScale)
            )
        ...
        FakeBackground.Size = UDim2.fromOffset((TitleX > DescX and TitleX or DescX) + ((24 + IconOffset) * Library.DPIScale), 0)
    end
...
function Library:CreateWindow(WindowInfo)
    ...
    local function ApplySidebarLayout()
        ...
        if LayoutRefs.WindowTitle then
            ...
            if not IsCompact then
                local MaxTextWidth =
                    math.max(0, SidebarWidth - (WindowInfo.Icon and 40 + 12 or 12))
                local TextWidth =
    ...
    do
        ...
        if WindowInfo.BackgroundImage then
            New("ImageLabel", {
                Image = typeof(WindowInfo.BackgroundImage) == "number"
                    and string.format("rbxassetid://%d", WindowInfo.BackgroundImage)
                    or WindowInfo.BackgroundImage,
                Position = UDim2.fromOffset(2, 2),
                Size = UDim2.new(1, -4, 1, -4),
                ScaleType = Enum.ScaleType.Fit,
                ZIndex = 1,
        ...
        local HasTitle = WindowInfo.Title ~= "" and WindowInfo.Title ~= nil
        WindowTitle = New("TextButton", {
            BackgroundTransparency = 1,
            Text = HasTitle and WindowInfo.Title or "",
            TextSize = 20,
            Visible = HasTitle and not LayoutState.IsCompact,
            Parent = TitleHolder,
        })
        if HasTitle and not LayoutState.IsCompact then
            local MaxTextWidth =
                math.max(0, InitialSidebarWidth - (WindowInfo.Icon and 40 + 12 or 12))
    ...
    function Window:SetBackgroundTransparency(transparency)
        ...
        if Tabs then
            Tabs.BackgroundTransparency = transparency
        end
        ...
        for _, obj in pairs(Library.Registry) do
            if obj.Instance and obj.BackgroundColor3 then
                local bgColor = obj.BackgroundColor3
                if bgColor == "BackgroundColor" then
                    obj.Instance.BackgroundTransparency = transparency
                elseif bgColor == "MainColor" then
                    obj.Instance.BackgroundTransparency = transparency * 0.5
                end
            end
        end
    end
    ...
    function Window:SetCornerRadius(radius)
        ...
        for _, obj in pairs(Library.Registry) do
            if obj.Instance then
                local corner = obj.Instance:FindFirstChildOfClass("UICorner")
                if corner then
        ...
        if mainCorner then
            mainCorner.CornerRadius = UDim.new(0, radius - 1)
        end
    end
    ...
    function Window:AddTab(...)
        ...
        do
            ...
            if Icon then
                -- Create background shape for icon
                local IconShape = Icon.Shape or "Circle"
                local ShapeFrame
                
                if IconShape == "Triangle" then
                    -- Triangle shape using ImageLabel
                    ShapeFrame = New("ImageLabel", {
                        AnchorPoint = Vector2.new(0, 0.5),
                        Position = UDim2.new(0, 0, 0.5, 0),
                        Size = UDim2.fromOffset(20, 20),
                        Image = "rbxassetid://14400935912",
                        ImageColor3 = Icon.CircleColor or "AccentColor",
                        ImageTransparency = Icon.CircleTransparency or 0.85,
                        BackgroundTransparency = 1,
                        Parent = TabButton,
                    })
                else
                    -- Square or Circle
                    ShapeFrame = New("Frame", {
                        AnchorPoint = Vector2.new(0, 0.5),
                        Position = UDim2.new(0, 0, 0.5, 0),
                        Size = UDim2.fromOffset(20, 20),
                        BackgroundColor3 = Icon.CircleColor or "AccentColor",
                        BackgroundTransparency = Icon.CircleTransparency or 0.85,
                        Parent = TabButton,
                    })
                    
                    if IconShape == "Circle" then
                        New("UICorner", {
                            CornerRadius = UDim.new(1, 0),
                            Parent = ShapeFrame,
                        })
                    elseif IconShape == "Square" then
                        New("UICorner", {
                ...
                TabIcon = New("ImageLabel", {
                    ...
                    Position = UDim2.fromScale(0.5, 0.5),
                    Image = Icon.Url,
                    ImageColor3 = Icon.Color or (Icon.Custom and "White" or "AccentColor"),
        ...
        function Tab:RefreshSides()
		    ...
		    TabLeft.Size = UDim2.new(0, math.floor(TabContainer.AbsoluteSize.X / 2) - 3, 1, -Offset)
		    ...
		    TabRight.Size = UDim2.new(0, math.floor(TabContainer.AbsoluteSize.X / 2) - 3, 1, -Offset)
        ...
        function Tab:AddGroupbox(Info)
            ...
            do
                ...
                -- Add chevron arrow
                local ArrowDownIcon = Library:GetCustomIcon("chevron-down")
                local ArrowRightIcon = Library:GetCustomIcon("chevron-right")
                
                local ArrowIcon = New("ImageLabel", {
                    AnchorPoint = Vector2.new(1, 0.5),
                    Image = ArrowDownIcon and ArrowDownIcon.Url or "",
                    ImageColor3 = "AccentColor",
                    ImageRectOffset = ArrowDownIcon and ArrowDownIcon.ImageRectOffset or Vector2.zero,
                    ImageRectSize = ArrowDownIcon and ArrowDownIcon.ImageRectSize or Vector2.zero,
            ...
            function Groupbox:SetCollapsed(State)
                ...
                if ArrowIcon then
                    if State then
                        ...
                        local ArrowRightIcon = Library:GetCustomIcon("chevron-right")
                        if ArrowRightIcon then
                            ArrowIcon.Image = ArrowRightIcon.Url
                            ArrowIcon.ImageRectOffset = ArrowRightIcon.ImageRectOffset
                            ArrowIcon.ImageRectSize = ArrowRightIcon.ImageRectSize
                        end
                    else
                        -- Expanded - show chevron-down
                        local ArrowDownIcon = Library:GetCustomIcon("chevron-down")
                        if ArrowDownIcon then
                            ArrowIcon.Image = ArrowDownIcon.Url
                            ArrowIcon.ImageRectOffset = ArrowDownIcon.ImageRectOffset
                            ArrowIcon.ImageRectSize = ArrowDownIcon.ImageRectSize
                        end
                    end
                end
        ...
        function Tab:AddTabbox(Info)
            ...
            function Tabbox:AddTab(Name, IconName)
                ...
                function Tab:Show()
                    ...
                    -- Update icon transparency if it exists
                    if TabIcon then
                        TweenService:Create(TabIcon, Library.TweenInfo, {
                            ImageTransparency = 0,
                        }):Play()
                    ...
                    -- Update text label transparency if icon exists
                    if IconName then
                        for _, child in ipairs(Button:GetChildren()) do
                            if child:IsA("TextLabel") then
                                TweenService:Create(child, Library.TweenInfo, {
