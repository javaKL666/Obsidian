...
do
    local BaseURL = "https://officialaxscripts.vercel.app/scripts/InksidianUI/assets/"
    local ConfigPath = "Inksidian/AssetManager.json"
    local AssetsPath = "Inksidian/Assets/"
    ...
    if Success and type(Data) == "table" then
        ...
        for AssetName, AssetData in pairs(AssetManager.Cache) do
    ...
    function AssetManager.GetAsset(AssetName: string)
        ...
        if not isfile(AssetData.Path) then
            AssetManager.DownloadAsset(AssetName)
            AssetData = AssetManager.Cache[AssetName]
        end
        ...
        if getcustomasset then
            ...
            if Success and NewID then
                ...
                AssetData = AssetManager.Cache[AssetName]
                if AssetData then
                    local RetrySuccess, RetryID = pcall(getcustomasset, AssetData.Path)
                    if RetrySuccess and RetryID then
                        AssetID = RetryID
                    end
                end
            end
    ...
    function AssetManager.DownloadAsset(AssetName: string)
        ...
        local AssetPath = AssetName .. ((AssetName == "Normal" or AssetName == "Error" or AssetName == "Toggle") and ".ogg" or ".png")
...
local AssetOverrides = {
    ...
    ["rbxassetid://6895079853"] = AssetManager.GetAsset("Toggle")
}
...
do
    local LibraryMetatable = {
        __index = function(t, key)
            if key == "ShowMobileLockButton" then
...
function Library:UpdateSearch(SearchText)
    ...
    if Library.LastSearchTab then
        for _, Groupbox in pairs(Library.LastSearchTab.Groupboxes) do
            ...
            local backup = Library.LastSearchTab._SearchBackupCollapsed
            if backup and Groupbox.SetCollapsed then
                local b = backup[Groupbox]
                if typeof(b) == "boolean" then
                    Groupbox:SetCollapsed(b)
                end
            end
    ...
    if Trim(Search) == "" or Library.ActiveTab.IsKeyTab then
        ...
        if Library.LastSearchTab and Library.LastSearchTab._SearchBackupCollapsed then
            Library.LastSearchTab._SearchBackupCollapsed = nil
        end
    ...
    for _, Groupbox in pairs(Library.ActiveTab.Groupboxes) do
        ...
        if VisibleElements > 0 then
            if Groupbox.Collapsed and Groupbox.SetCollapsed then
                local backup = Library.ActiveTab._SearchBackupCollapsed
                if not backup then
                    Library.ActiveTab._SearchBackupCollapsed = {}
                    backup = Library.ActiveTab._SearchBackupCollapsed
                end
                if backup[Groupbox] == nil then
                    backup[Groupbox] = true
                end
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        function KeyPicker:Update()
            ...
            if KeybindsToggle.Loaded then
                ...
                if KeyPicker.Value == "None" or KeyPicker.Value == "" or KeyPicker.Value == "Unknown"
                    or KeyPicker.DisplayValue == "None" or KeyPicker.DisplayValue == "" or KeyPicker.DisplayValue == "Unknown" then
                    KeybindsToggle:SetVisibility(false)
                else
                    if ShowToggle then
...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        Library:MakeOutline(MainFrame, WindowInfo.CornerRadius, 0, true)
    ...
    function Window:AddTab(...)
        ...
        function Tab:AddGroupbox(Info)
            ...
            do
                ...
                local ChevronIcon = ChevronIconData and New("ImageLabel", {
                    Name = "ReplicatedStorage",
                ...
                HeaderButton.MouseButton1Click:Connect(function()
                    ...
                    local gb = Tab.Groupboxes[Info.Name]
                    if gb and gb.SetCollapsed then
                        gb:SetCollapsed(not gb.Collapsed)
                    end
                end)
            ...
            function Groupbox:SetCollapsed(state: boolean)
                ...
                if state then
                    Background.Size = UDim2.new(1, 0, 0, 53 * Library.DPIScale)
                else
                    Groupbox:Resize()
                end
            end
...
function Library:SetGroupboxesCollapsible(state: boolean)
